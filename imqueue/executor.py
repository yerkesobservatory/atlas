...
class Executor(base.AtlasServer):
    ...
    def wait_until_good(self) -> bool:
        ...
        max_wait = config.queue.max_wait_time
        ...
        while weather is False:
            ...
            self.slack('Executor is sleeping for 15 minutes', '#queue')
            time.sleep(60*config.queue.wait_time) # time to sleep in seconds
            elapsed_time += (60*config.queue.wait_time)

            # shut down after 4 hours of continuous waiting
            if elapsed_time >= (60*60*max_wait):
                self.log('Bad weather for {} hours. Shutting down the queue...'.format(max_wait),
                         color='magenta')
                self.slack('Bad weather for {} hours. Shutting down the queue...'.format(max_wait),
                           '#queue')
        ...
        self.slack('Weather is currently good.', '#queue')
        return True
    ...
    def execute_queue(self) -> bool:
        ...
        """
        self.slack('Starting execution of the queue...', '#queue')
        ...
        # wait for the weather to be good
        try:
            self.wait_until_good()
        except WeatherException:
            return False
        except Exception as e:
            self.log('Unknown exception occured while waiting for weather...', color='red')
            self.slack('Unknown exception occured while waiting for weather...', '#queue')
        ...
        self.telescope.keep_open(3600)

        # load queues from database
        self.sessions = self.load_sessions()
        self.slack('Queue has {} unexecuted sessions'.format(len(self.sessions)), '#queue')
        ...
        self.log('Building RA,Dec coordinates for queue')
        objects = []
        for s in self.sessions:
            ...
            try:
                ...
                self.slack('find_target unable to find object. Skipping...', '#queue')
                continue
        ...
        while len(self.sessions) != 0:
            ...
            try:
                ...
                if location != '':
                    ...
                    self.slack('Sending location to pipeline: {}...'.format(location), '#queue')
                ...
                self.log('Error while executing session {}'.format(session.id),
                         color='red')
                self.slack('An exception occured while executiong session {}'.format(session.id), '#queue')
    ...
    def execute(self, session: dict, ra: str, dec: str) -> str:
        ...
        try:
            ...
            self.slack('Succesfully made directory to store files: {}'.format(dirname), '#queue')
            ...
            if good_pointing is False:
                ...
                self.slack('Pinpoint failed!', '#queue')
            ...
            for filt in filters:
                ...
                if self.telescope.dome_open() is False:
                    self.slack('Dome closed unexpectedly. Opening up', '#queue')
                    self.telescope.open_dome()
            ...
            # take numbias*exposure_count biases
            self.slack('Taking bias frames...', '#queue')
    ...
    def take_exposures(self, basename: str, exp_time: int,
        ...
        while i < count: 
            ...
            if self.telescope.dome_open() is False:
                ...
                self.slack('Slit closed during exposure - repeating previous exposure!', '#queue')
                self.wait_until_good()
                self.telescope.open_dome()
                continue
    ...
    def finish(self):
        ...
        if self.telescope is not None:
            self.slack('Closing down the telescope and disconnecting...', '#queue')
            self.telescope.close_down()
